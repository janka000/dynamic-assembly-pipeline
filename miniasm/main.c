...
static void print_hits(size_t n_hits, const ma_hit_t *hit, const sdict_t *d, const ma_sub_t *sub, const char *out_hit_fn)
{
	fprintf(stderr,"printing hits to paf (%s)\n", out_hit_fn);
	size_t i;
	FILE *fp = fopen(out_hit_fn,"w");
	if (fp == NULL){
		fprintf(stderr,"Error opening the file %s", out_hit_fn);
	}
	else{
		for (i = 0; i < n_hits; ++i) {
			const ma_hit_t *p = &hit[i];
			const ma_sub_t *rq = &sub[p->qns>>32], *rt = &sub[p->tn];
			fprintf(fp,"%s:%d-%d\t%d\t%d\t%d\t%c\t%s:%d-%d\t%d\t%d\t%d\t%d\t%d\t255\n", d->seq[p->qns>>32].name, rq->s + 1, rq->e, rq->e - rq->s, (uint32_t)p->qns, p->qe,
...
static void print_reads(const sdict_t *d, const char *out_read_fn)
{
	fprintf(stderr,"printing read names (%s)\n", out_read_fn);
	size_t i;
	FILE *fp = fopen(out_read_fn,"w");
	if (fp == NULL){
		fprintf(stderr,"Error opening the file %s", out_read_fn);
	}
	else{
		for (i = 0; i < d->n_seq; ++i) {
		   if (!(d->seq[i].del)) {
...
int main(int argc, char *argv[])
{
	...
	sdict_t *d, *excl = 0, *low_coverage = 0; //sequence dictionary (d - all , excl - to be excluded - containments in the "0th" step)
	ma_sub_t *sub = 0;
	ma_hit_t *hit;
	size_t n_hits;
	float cov = 40.0;
	char *fn_reads = 0, *outfmt = "ug";
	char *out_hit_fn = "filtered.paf";
	char *out_read_fn = "filtered.txt";

	ma_opt_init(&opt);
	while ((c = getopt(argc, argv, "n:m:s:c:S:i:d:g:o:h:I:r:f:e:p:a:j:12VBRbF:")) >= 0) {
		...
		else if (c == 'S') stage = atoi(optarg); //minimap can continue from specified stage!!
		...
		else if (c == 'a') out_hit_fn = optarg; 
		else if (c == 'j') out_read_fn = optarg; 
...
//		fprintf(stderr, "    -B          only one direction of an arc is present in input PAF\n"); // deprecated; for backward compatibility
	...
	if (no_cont) {
		...
		excl = ma_hit_no_cont(argv[optind], opt.min_span, opt.min_match, opt.max_hang, opt.int_frac); //build a hash table of contained reads
	}


    //** steps 1-3 use functions form hit.c */

	fprintf(stderr, "[M::%s] ===> Step 1: reading read mappings <===\n", __func__);
	hit = ma_hit_read(argv[optind], opt.min_span, opt.min_match, d, &n_hits, bi_dir, excl); //creates (and returns) an array of MAPPINGS from PAF file, stores the READS form them into the hash table and counts the mappings stored
	//hit -> pointer to array of mappings - mappings that are NOT contained  AND  passed filters for span and match length
	//n_hits -> # of stored mappings
	//d -> hash table of reads in the mappings
	//excl -> hash table of contained reads (exclude)

	if (!no_first) {
		fprintf(stderr, "[M::%s] ===> Step 2: 1-pass (crude) read selection <===\n", __func__);
		if (stage >= 2) {
			sub = ma_hit_sub(opt.min_dp, opt.min_iden, 0, n_hits, hit, d->n_seq); //filtering based on coverage and identity
			n_hits = ma_hit_cut(sub, opt.min_span, n_hits, hit); //actual modifications in the hit array based on the sub array (considering strands, min span..)
		}
		if (stage >= 3) n_hits = ma_hit_flt(sub, opt.max_hang * 1.5, opt.min_ovlp * .5, n_hits, hit, &cov); //convert hits to arcs, check for containments, keep overlaps only, exclude hits associated with deleted regions
	}
	...
	if (!no_second) {
		...
		if (stage >= 4) {
			ma_sub_t *sub2;
			sub2 = ma_hit_sub(opt.min_dp, opt.min_iden, opt.min_span/2, n_hits, hit, d->n_seq); //same as in previous step, but min_span/2 clipped from ends of the mapping.. 
			n_hits = ma_hit_cut(sub2, opt.min_span, n_hits, hit);
			if (!no_first) {
				ma_sub_merge(d->n_seq, sub, sub2); //merge the sub arrays
				free(sub2);
			} else {
				sub = sub2;
			}
		}
		if (stage >= 5) n_hits = ma_hit_contained(&opt, d, sub, n_hits, hit);//remove containments, return num of remaining hits
	}

	hit = (ma_hit_t*)realloc(hit, n_hits * sizeof(ma_hit_t));

    //-p STR    Output information and format [ug]. 
	//Possible STR values include 
	...
	if (strstr(outfmt, "bed") != NULL) {
		print_subs(d, sub);
	} 
	if (strstr(outfmt, "paf") != NULL) { 
		print_hits(n_hits, hit, d, sub, out_hit_fn);
	} 
	if (strstr(outfmt, "reads") != NULL) { 
		print_reads(d, out_read_fn);
	} 
	if (strstr(outfmt, "ug") != NULL || strstr(outfmt, "sg") != NULL) {
		asg_t *sg = 0;
		ma_ug_t *ug = 0;
