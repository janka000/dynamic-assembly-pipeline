...
/**
 * reads PAF file line by line
 * @param paf_file_t *pf  PAF file (pointer+buffer in struct)
 * @param paf_rec_t *r current line ?
*/
...
int paf_read(paf_file_t *pf, paf_rec_t *r)
{
	int ret, dret;
file_read_more:
	ret = ks_getuntil((kstream_t*)pf->fp, KS_SEP_LINE, &pf->buf, &dret); //get line
	if (ret < 0) return ret;
	ret = paf_parse(pf->buf.l, pf->buf.s, r); //parse line from PAF file - store to r (but only pointers to name strings are stored..)
