...
typedef struct {
	uint64_t qns; //query name + start packed into one int (?)
	uint32_t qe, tn, ts, te; //query end, target name, target start, terget end
	uint32_t ml:31, rev:1; //num matches, relative strand
	uint32_t bl:31, del:1; //alignment block length
} ma_hit_t;

typedef struct { size_t n, m; ma_hit_t *a; } ma_hit_v; //https://github.com/attractivechaos/klib, kvec.h
...
/**
 *  converts a sequence mapping hit to an overlap arc, considering conditions for internal overlaps, containment, and short overlaps
 * @returns length of overlap
 * also modifies the asg_arc_t *p
*/
static inline int ma_hit2arc(const ma_hit_t *h, int ql, int tl, int max_hang, float int_frac, int min_ovlp, asg_arc_t *p)
{
	...
	// calculate overhangs on the 5' and 3' ends of the alignment
	if (h->rev) tl5 = tl - h->te, tl3 = h->ts; // tl5: 5'-end overhang (on the query strand); tl3: similar
	else tl5 = h->ts, tl3 = tl - h->te;
	ext5 = qs < tl5? qs : tl5; //minimum of query start and 5'-end overhang
	ext3 = ql - h->qe < tl3? ql - h->qe : tl3; //minimum of (query length - query end) and 3'-end overhang
	if (ext5 > max_hang || ext3 > max_hang || h->qe - qs < (h->qe - qs + ext5 + ext3) * int_frac)
		return MA_HT_INT; //internal
	if (qs <= tl5 && ql - h->qe <= tl3) return MA_HT_QCONT; // query contained
	else if (qs >= tl5 && ql - h->qe >= tl3) return MA_HT_TCONT; // target contained
	//determine the direction of the arc (u to v) and the length of the overlap
